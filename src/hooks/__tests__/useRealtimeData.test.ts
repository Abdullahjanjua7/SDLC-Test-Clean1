...
describe('useRealtimeData', () => {
  ...
  afterEach(() => {
    ...
    jest.runOnlyPendingTimers(); // Ensure all pending timers are run before restoring
    jest.useRealTimers();
  });

  // 1. Test the hook's initial state
  test('should return initial state with undefined data and loading true by default', () => {
    mockFetcher.mockResolvedValueOnce('test data');
    const { result } = renderHook(() => useRealtimeData(mockFetcher));

    expect(result.current.data).toBeUndefined();
    expect(result.current.loading).toBe(true); // It starts loading immediately
